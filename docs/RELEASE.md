...
> 当前 Release：`https://github.com/anmili2022/RouletteRecorder.Dalamud/releases/tag/v1.0.6.1`
...
```powershell
Set-Content -Encoding UTF8 output\release_notes.md "# 日随伴侣 v1.0.6.1`n`n## 主要更新`n`n- 写这里。`n- 写这里。`n`n## 构建产物`n`n- latest.zip：Dalamud 插件发布包。"
```
...
```powershell
git add .
git commit -m "chore: release v1.0.6.1"
```

打标签：

```powershell
git tag -a v1.0.6.1 -m "日随伴侣 v1.0.6.1"
```
...
```powershell
gh release create v1.0.6.1 output\RouletteBuddy\latest.zip --title "日随伴侣 v1.0.6.1" --notes-file output\release_notes.md
```

如果 Release 已经存在，需要覆盖上传包：

```powershell
gh release upload --clobber v1.0.6.1 output\RouletteBuddy\latest.zip
```

查看发布结果：

```powershell
gh release view v1.0.6.1 --json tagName,name,url,assets,publishedAt
```
...
```text
https://github.com/anmili2022/RouletteRecorder.Dalamud/releases/download/v1.0.6.1/latest.zip
```
